    ...
    <div class="both-sections-container d-lg-flex ">
      ...
      <div class="section-one mt-5 mb-0">
        ...
        <div class="row mt-1 featured-videos">
          ...
          @if($homevideoall->count() > 0)
          @foreach($homevideoall as $index => $values)
    <div class="col-12 col-md-4 col-lg-4 mt-3 col-xl-4">
        <div class="card video-card ">
            <iframe class="iframe" width="100%" height="100%" id="playvideo{{$index}}" src="{{$values->video_link}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen style="display: none;"></iframe>

            <div class="position-relative d-inline-block">
                <img src="{{ asset('videosimages/'.$values->video_image) }}" class="recentblog-image changeimageid" height="100%" width="100%">
                <span id="youtube-btn-icon-span-tag{{$index}}" class="position-absolute bottom-0 youtube-btn-icon-span-tag end-0 m-1 bg-dark text-light p-2 rounded-circle">
                    <i class="fab fa-youtube youtubebtnicon"></i>
                </span>
            </div>
...
@if(method_exists($homevideoall,'links'))
   $homevideoall->links();
@endif
          ...
          <span class=" mt-5 text-center"><span class="text-center bg-dark text-white py-3 mt-5 px-5 rounded-pill">No Results Found.</span></span>
          </div>
          @endif
        </div>
        <!-- row end -->
    ...
    <div class="position-relative d-inline-block mt-2"  style="height: 80px; width:100%">
                ...
                <img src="{{ asset('videosimages/'.$topTrendingValues->video_image) }}" class="changeimageidtwo" width="100%" height="100%" alt="{{$topTrendingValues->video_title}}" />
                </a>
                <a href="{{$topTrendingValues->video_link}}">
...
@foreach($videoCategory as $videoCategoryValues)
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function () {
        var changeimageidImages = document.querySelectorAll('.changeimageid');

        changeimageidImages.forEach(function(image, index) {
            image.addEventListener("click", function() {
                var playvideo = document.getElementById('playvideo' + index);
                var youtubeBtnIcon = document.getElementById('youtube-btn-icon-span-tag' + index);

                playvideo.style.display = "block";
                image.style.display = "none";
                youtubeBtnIcon.style.display = "none";
            });
        });
    });
</script>
